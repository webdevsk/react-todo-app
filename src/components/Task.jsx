...
export default function Task({task}){
  return (
    <div className={`task-item bg-white dark:bg-gray-800 flex items-center border-solid border border-gray-200 dark:border-gray-800 rounded-lg shadow-md relative overflow-hidden animate-toUp [animation-fill-mode:forwards] -z-0`}>

        {/* Completed marker. Use Border color class */}
        <CompletedMarker {...task} className="border-amber-500" color="amber" />

        {/* Animate when checked */}
        <span className={`absolute left-0 -translate-x-2/4 top-2/4 -translate-y-2/4 bg-gray-200 dark:bg-gray-900 rounded-full w-0 h-0 origin-bottom -z-10 [animation-fill-mode:forwards] ${task.completed ? 'animate-ripple' : ''}`} ></span>
