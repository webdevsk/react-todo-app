...
function ImportantMarker({inputClasses, shadowClasses, iconClasses, task, handleUpdateTask}) {
  ...
  <div className={`inputContainer relative mx-6`}>
    <Tooltip content="Mark as important" animate={{ mount: { scale: 1, y: 0 }, unmount: { scale: 0, y: 25 }}}>
      <input type="checkbox" checked={important} onChange={handleImportant} className={`opacity-0 cursor-pointer w-12 h-12 absolute top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 rounded-full peer ${inputClasses}`} />
    </Tooltip>
